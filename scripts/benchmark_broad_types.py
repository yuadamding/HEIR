...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.inspect:
        report = inspect_broad_type_gate(Path(args.plan))
        atomic_json_dump(report, args.output)
        print(json.dumps(report, indent=2, sort_keys=True))
        return 0
    try:
        report = run_broad_type_gate(Path(args.plan), device_name=args.device)
    except (BroadTypeGateBlocked, FileNotFoundError, ValueError) as error:
        report = blocked_broad_type_report(Path(args.plan), error)
        atomic_json_dump(report, args.output)
        print(json.dumps(report, indent=2, sort_keys=True))
        return 2
    atomic_json_dump(report, args.output)
