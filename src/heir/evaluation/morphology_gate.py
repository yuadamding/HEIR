...
MORPHOLOGY_RIDGE_REPORT_SCHEMA = "heir.morphology_ridge_evaluation.v4"
...
def _evaluate_permutation_null(
    ...
    *,
    ...
    if len(values) != total_permutations:
        raise ValueError(
            "%s cannot generate the prespecified number of unique active permutations"
            % null_kind
        )
    ...
    minimum_cross_block = (
        float(min(cross_block_fractions)) if cross_block_fractions else None
    )
    activity_pass = minimum_changed >= minimum_shuffled_fraction and (
        minimum_cross_block is None
        or minimum_cross_block >= minimum_shuffled_fraction
    )
    delta = float(matched - array.mean())
    pool_pass = bool(
        activity_pass
        and delta >= minimum_shuffle_delta
    ...
    activity = (
        null_stratum_activity(
            development.donor_ids, development.type_labels, development.roi_ids
        )
        if null_kind == "local_within_roi"
...
def evaluate_morphology_ridge_gate(
    ...
    *,
    ...
    calibration = validate_calibration_receipt(
        calibration_receipt, required=final_inference
    )
    ...
    stratum_macro = float(
        np.mean([float(row["residual_coordinate_r2"]) for row in rows])
    )
    ...
    best_nuisance_macro, best_nuisance_donors, *_ = available_nuisance[
        best_nuisance_name
    ]
    ...
    local_null_effects = paired_donor_effects(
        donor_macro, local_null["donor_null_mean_r2"]
    )
    block_null_effects = paired_donor_effects(
        donor_macro, block_null["donor_null_mean_r2"]
    )
    per_donor_effects = {
        donor: {
            ...
            "matched_minus_best_nuisance_macro_r2": float(
                best_nuisance_effects[donor]
            ),
    ...
    section_ids = getattr(locked_test, "section_ids", None)
    section_source = "explicit_artifact_metadata"
    if section_ids is None:
        section_ids = section_ids_from_blocks(locked_test.block_ids)
        section_source = "parsed_from_block_id"
    section_coverage = (
        donor_section_type_coverage(
            locked_test.donor_ids,
            section_ids,
    ...
    section_stratification = (
        ...
        if section_ids is not None
            ...
            "available": False,
            "reason": "section identity is unavailable in artifact metadata and block_ids",
        }
    )
    disease_labels = getattr(locked_test, "disease_states", None)
    disease_stratification = (
        ...
        if disease_labels is not None
            ...
            "available": False,
            "reason": "disease labels are not carried by the compact evaluation artifact",
        }
    )
    rank_sensitivity = []
    ...
    checks = {
        ...
        "paired_best_nuisance_effect_ci_positive": (
            best_nuisance_bootstrap["ci_95"][0] > 0.0
        ),
    ...
    if final_inference:
        checks["source_coverage_audit_available"] = bool(
            getattr(locked_test, "coverage_audit", None) is not None
        )
            ...
            "composition_adjusted": (
                {
                    ...
                    "uni2h_minus_best_nuisance_macro_r2": (
                        matched - best_nuisance_macro
                    ),
                }
                if include_composition
                else None
    ...
    checks["every_seed_separates_shuffle"] = checks[
    ...
    return {
        ...
        "best_independently_tuned_nuisance_control": {
            ...
            "exact_donor_paired_randomization": exact_paired_randomization(
                best_nuisance_effects
            ),
        ...
        "primary_metrics": {
            ...
            "matched_minus_coordinate_donor_bootstrap_ci_95": coordinate_bootstrap[
                "ci_95"
            ],
            "matched_minus_stain_donor_bootstrap_ci_95": (
                stain_bootstrap["ci_95"] if stain_bootstrap is not None else None
            ),
            "matched_minus_best_nuisance_donor_bootstrap_ci_95": (
                best_nuisance_bootstrap["ci_95"]
            ),
        },
        ...
        "leave_one_locked_donor_out": {
            ...
            "matched_minus_coordinate_macro_r2": leave_one_donor_out(
                coordinate_effects
            ),
            "matched_minus_stain_macro_r2": (
                leave_one_donor_out(stain_effects) if stain_macro is not None else None
            ),
            "matched_minus_best_nuisance_macro_r2": leave_one_donor_out(
                best_nuisance_effects
            ),
        },
        "donor_bootstrap": {
        ...
        "exact_donor_paired_randomization": {
            ...
            "matched_minus_best_nuisance": exact_paired_randomization(
                best_nuisance_effects
            ),
        },
        "donor_dominance": {
